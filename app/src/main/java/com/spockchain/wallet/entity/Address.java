...
public class Address {
    ...
    private static final Pattern ignoreCaseSpockAddrPattern = Pattern.compile("(?i)^(SPOCK-)?[0-9a-f]{40}$");
    private static final Pattern addrPattern = Pattern.compile("^(0x)?[0-9a-fA-F]{40}$");
    private static final Pattern spockAddrPattern = Pattern.compile("^(0x)?[0-9a-fA-F]{40}$");
    ...
    public static boolean isSpockAddress(String address) {
        return ignoreCaseSpockAddrPattern.matcher(address).find();
    }
